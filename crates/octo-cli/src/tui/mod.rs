...
use octo_core::permission::PermissionDecision;
...
struct TuiApp {
    app: super::App,
    session: octo_core::session::Session,
...
pub async fn run(
    ...
    let session = match resume_session {
        ...
        None => {
            let s = octo_core::session::Session::new("New session".into());
            app.db
                .sessions()
                .create(&s)
                .await
...
async fn handle_agent_event(app: &mut TuiApp, event: AgentEvent) {
    match event {
        ...
        AgentEvent::Complete { usage, .. } => {
            ...
            if let Some(m) = octo_core::model::get_model(app.app.agent.model_id()) {
...
fn switch_model(app: &mut TuiApp, model_id: &octo_core::model::ModelId) {
    match octo_providers::create_provider(&app.app.config, Some(model_id)) {
        Ok(p) => {
            ...
            if let Ok(db_msgs) = app.app.db.messages().list(session_id).await {
                for msg in &db_msgs {
                    ...
                    let role = match msg.role {
                        octo_core::message::MessageRole::User => ChatRole::User,
                        octo_core::message::MessageRole::Assistant => ChatRole::Assistant,
                        octo_core::message::MessageRole::System => ChatRole::System,
                        octo_core::message::MessageRole::Tool => ChatRole::Tool("tool".into()),
                    };
